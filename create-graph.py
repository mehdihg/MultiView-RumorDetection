...
tweet=[]
label=[]
df = pd.read_excel(r"/content/MultiView-RumorDetection/shufflefilenew2.xlsx")
#print(str(df.values[0][1]))
tweetcred=np.zeros((len(df),1))
for i in range(0,len(df)):
    filee=open('/content/MultiView-RumorDetection/nontrue15162/'+str(df.values[i][1]),'r')
    ...
    for j in range(i+1,len(df)):
      fileee=open('/content/MultiView-RumorDetection/nontrue15162/'+str(df.values[j][1]),'r')
